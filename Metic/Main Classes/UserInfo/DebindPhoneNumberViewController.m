...
@interface DebindPhoneNumberViewController ()

@end

@implementation DebindPhoneNumberViewController
...
- (IBAction)debindPhone:(id)sender {
    ...
    [SMSSDK commitVerificationCode:verificationCode phoneNumber:self.phoneTextField.text zone:@"+86" result:^(NSError *error) {
        if (!error) {
            NSLog(@"验证成功");
            [SVProgressHUD showWithStatus:@"正在解绑手机，请稍候" maskType:SVProgressHUDMaskTypeBlack];
            [self debindPhoneNumber:phoneNumber];
        } else {
            NSLog(@"验证失败");
...
- (void)debindPhoneNumber:(NSString *)phoneNumber{
    ...
    [MTAccountManager bindPhoneWithUserId:[MTUser sharedInstance].userid phoneNumber:phoneNumber password:nil toBind:MTPhoneBindSatausToDebind success:^{
        ...
        if (account.type == MTAccountTypePhoneNumber) {
            account.type = MTAccountTypeEmpty;
        }
        [account saveAccount];
