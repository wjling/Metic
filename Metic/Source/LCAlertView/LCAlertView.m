...
#define kCancleColor [UIColor colorWithWhite:200.0/255.0 alpha:1.0]
...
- (void)setupFrame
{
    float contentHeight = (self.message && ![self.message isEqualToString:@""])? 60:15;
    switch(_allBtnNum)
    {
        case 0:
        {
...
//        }
        default:
        {
            float height = kTitleYOffset+kTitleHeight+contentHeight+_buttons.count*kButtonHeight+kButtonBottomOffset;
    ...
    _alertContentLabel.frame = CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, CGRectGetMaxY(_alertTitleLabel.frame), contentLabelWidth, contentHeight);
...
- (void)show
{
    ...
    UITapGestureRecognizer * singleRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(defaultHideView)];
