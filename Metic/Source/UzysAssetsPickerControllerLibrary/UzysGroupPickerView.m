...
- (void)setupLayout
{
    ...
    self.frame = CGRectMake(0, - [UIScreen mainScreen].bounds.size.height, kMainScreenWidth, [UIScreen mainScreen].bounds.size.height);;
...
- (void)setupTableView
{
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NavigationHeight, kMainScreenWidth, self.bounds.size.height -NavigationHeight) style:UITableViewStylePlain];
...
- (void)show
{
    [UIView animateWithDuration:0.25f delay:0.f options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionBeginFromCurrentState animations:^{
        self.frame = CGRectMake(0, BounceAnimationPixel , kMainScreenWidth, [UIScreen mainScreen].bounds.size.height);
            ...
            [UIView animateWithDuration:0.15f delay:0.f options:UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionBeginFromCurrentState animations:^{
                    self.frame = CGRectMake(0, 0 , kMainScreenWidth, [UIScreen mainScreen].bounds.size.height);
            } completion:^(BOOL finished) {
            }];
    }];
    self.isOpen = YES;
